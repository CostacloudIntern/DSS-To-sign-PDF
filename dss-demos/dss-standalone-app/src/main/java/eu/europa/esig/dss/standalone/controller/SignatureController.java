...
//				  fileChooser.getExtensionFilters().add( new FileChooser.ExtensionFilter("PKCS11 library (*.dll)", "*.dll")); 
//				  } else
				 
				if (SignatureTokenType.PKCS12.equals(model.getTokenType())) {
					fileChooser.setTitle("Keystore");
					fileChooser.getExtensionFilters()
							.add(new FileChooser.ExtensionFilter("PKCS12 keystore (*.p12, *.pfx)", "*.p12", "*.pfx"));
				}
		...
		BooleanBinding isPkcs11Or12 = //model.tokenTypeProperty().isEqualTo(SignatureTokenType.PKCS11).or
				(model.tokenTypeProperty().isEqualTo(SignatureTokenType.PKCS12));
...
//				.and(model.signaturePackagingProperty().isNull());
		...
		BooleanBinding isPKCSIncomplete = //model.tokenTypeProperty().isEqualTo(SignatureTokenType.PKCS11).or
				(model.tokenTypeProperty().isEqualTo(SignatureTokenType.PKCS12)).and(isEmptyFileOrPassword);
...
//		SignatureTokenType tokenType = (SignatureTokenType) newValue.getUserData();
//		
//		  sigTokenTypeSupportedDigestAlgorithms = new
//		  ArrayList<>(supportedDigestAlgorithms);
//		  
//		  switch (tokenType) { case MSCAPI: // SHA224 not supported
